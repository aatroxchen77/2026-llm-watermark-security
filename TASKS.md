...
- [ ] 4a 写脚本: 从生成文本中提取绿名单命中序列(1,0,1,1,0...) → 计算 Lag-1 自相关系数
- [ ] 4b 补充分析到 3.2 节: 若自相关弱 → 用数据证明 Z 检验仍适用；若较强 → 提 Block Bootstrap 作为替代
- [ ] 4c 推 git → biolab 跑 → 拉结果

### R6 — Rational Adversary 敏感性曲线

评审指出：只有一个数据点(9.11%→1.66)，不够。

- [ ] 6a 写脚本: for 循环 0%-20% 替换率(步长 2%) + 随机替换 baseline
- [ ] 6b 画折线图: X=替换率, Y=Z-score, 两条线(定向替换 vs 随机替换)
- [ ] 6c 插入论文: 新图 + 分析段
- [ ] 6d 推 git → biolab 跑 → 拉结果

### R3 — 置信区间 (Bootstrap)

评审核心批评：所有指标都是单点估计，无不确定性量化。

- [ ] 3a 写脚本: 对 TVD/Z-score/PPL/检测率做 bootstrap (1000 次重采样, 95% CI)
- [ ] 3b 更新 Table 1: Z-score 从 "9.92" → "9.92 ± 0.45" 格式
- [ ] 3c 更新 6.1 节: TVD 从 "0.2809" → "0.2809 [95% CI: 0.26, 0.30]"
- [ ] 3d 推 git → biolab 跑 → 拉结果
...
- [ ] S1 放弃声明: 在 Limitations 中写"受限于计算资源，仅在 OPT-1.3B 上验证，未来应在 LLaMA-2 等更大规模模型上测试"
- [ ] S3 放弃声明: DSC 模型假设独立/随机翻转，不直接适用于理性博弈者
- [ ] S7 放弃声明: 缺乏人类评估文本质量
